...
const LATITUDE = 25.034048; // Taipei 101
const LONGITUDE = 121.564742; // Taipei 101
...
class DisplayLatLng extends React.Component {
  ...
  getES() {
    fetch('http://localhost:9200/tw_cities_points/_search?pretty&size=10', ELASTICSEARCH_QUERY)
    .then((resp) => resp.json())
    .then((respJson) => {
      hits = respJson.hits.hits;
      source = hits[hits.length-1]._source;

      name = source.name;
      lon = source.location[0];
      lat = source.location[1];

      message = name + ': ' + lon + ', ' + lat;
      ...
      <View style={styles.container}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity
            ...
            style={[styles.bubble, styles.button]}
          >
            <Text style={styles.buttonText}>getES</Text>
          </TouchableOpacity>
          <TouchableOpacity
